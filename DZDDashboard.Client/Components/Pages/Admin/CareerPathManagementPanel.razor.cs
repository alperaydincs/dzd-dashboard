...
public partial class CareerPathManagementPanel
{
    ...
    private sealed class RuleEditModel
    {
        ...
        public decimal?                 SalaryIncreasePercent;
        public decimal?                 PrivatePensionAmount;
        public string                   PrivatePensionCurrency = Currencies.Try;
        public decimal?                 EmployerContribAmount;
        public string                   EmployerContribCurrency = Currencies.Try;
        public decimal?                 MealAllowanceAmount;
        public string                   MealAllowanceCurrency = Currencies.Try;
    }
    ...
    {
        if (await DialogServiceRef.ShowMessageBoxAsync(Loc["gradeManagement.deleteCareerPathTitle"],
    ...
    private void BeginEditPath(CareerPathDto path)
    {
        ...
        _editRules = path.Rules
            ...
            .Select(r => new RuleEditModel
            {
                ...
                SalaryIncreasePercent   = r.SalaryIncreasePercent,
                PrivatePensionAmount    = r.PrivatePensionInsuranceAmount,
                PrivatePensionCurrency  = r.PrivatePensionInsuranceCurrency ?? Currencies.Try,
                EmployerContribAmount   = r.EmployerContributionUpperLimitAmount,
                EmployerContribCurrency = r.EmployerContributionUpperLimitCurrency ?? Currencies.Try,
                MealAllowanceAmount     = r.MealAllowanceAmount,
                MealAllowanceCurrency   = r.MealAllowanceCurrency ?? Currencies.Try,
            })
            .ToList();
    ...
    private async Task SavePathEdit()
    {
        ...
        try
        {
            ...
            {
                var delResp = await OrgService.DeleteCareerPathRuleAsync(removedId);
            ...
            foreach (var r in _editRules)
            {
                ...
                var resp = r.Id == 0
                    ? await OrgService.CreateCareerPathRuleAsync(dto)
                    : await OrgService.UpdateCareerPathRuleAsync(dto);
                if (!resp.IsSuccessStatusCode)
                {
    ...
    private static CareerPathRuleDto ToRuleDto(RuleEditModel r, int pathId) => new()
    {
        Id                           = r.Id,
        CareerPathId                 = pathId,
        Grade                        = r.Grade,
        ...
        PrivatePensionInsuranceAmount   = r.PrivatePensionAmount,
        PrivatePensionInsuranceCurrency = r.PrivatePensionAmount is null ? null : r.PrivatePensionCurrency,

        EmployerContributionUpperLimitAmount   = r.EmployerContribAmount,
        EmployerContributionUpperLimitCurrency = r.EmployerContribAmount is null ? null : r.EmployerContribCurrency,

        MealAllowanceAmount   = r.MealAllowanceAmount,
        MealAllowanceCurrency = r.MealAllowanceAmount is null ? null : r.MealAllowanceCurrency,
    };


    private static string FormatSalaryIncrease(CareerPathRuleDto r) =>
        r.SalaryIncreasePercent is { } pct ? $"{pct:0.##}%" : "—";

    private static string FormatMoney(decimal? amount, string? currency) =>
        amount is { } a ? $"{Currencies.Symbol(currency ?? Currencies.Try)}{a:#,##0.##}" : "—";

    private static string FormatMinExperience(CareerPathRuleDto r) =>
